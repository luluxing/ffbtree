...
using namespace btreemem;

// Define a parameterized test fixture
class BTreeMemTest : public ::testing::TestWithParam<InsertStrategy> {
 protected:
  void SetUp() override {}
...
TEST_P(BTreeMemTest, InitConstruction) {
  BTreeMem<int, int> btree(GetParam());
  EXPECT_EQ(btree.get_height(), 1);
}

TEST_P(BTreeMemTest, InsertAndLookupSingle) {
  BTreeMem<int, int> btree(GetParam());
  EXPECT_EQ(btree.get_height(), 1);
  btree.insert(1, 10);
  int val;
...
TEST_P(BTreeMemTest, InsertAndLookupSequential) {
  BTreeMem<int, int> btree(GetParam());
  EXPECT_EQ(btree.get_height(), 1);
  int num = 50000;
...
TEST_P(BTreeMemTest, InsertAndLookupRand) {
  BTreeMem<int, int> btree(GetParam());
  EXPECT_EQ(btree.get_height(), 1);
  int num = 519*260+520;
  std::vector<int> data;
...
TEST(BTreeMemTest, BulkLoadAndLookup) {

}

TEST(BTreeMemTest, RangeScan) {
