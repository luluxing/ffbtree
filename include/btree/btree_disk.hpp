...
public:
  int insert(BTreeDisk<KeyType, ValueType>& btree, 
              KeyType& key, ValueType& val,
              std::stack<int>& path) override {
    return btree.fsplit1_insert(key, val, path);
  }
};
...
public:
  int insert(BTreeDisk<KeyType, ValueType>& btree, 
              KeyType& key, ValueType& val,
              std::stack<int>& path) override {
    return btree.ssplit_insert(key, val, path);
  }
};
...
class BTreeDisk {
 ...
 public:
  BTreeDisk(const char *file_name, int cache_size,
  ...
  int insert(KeyType key, ValueType val) {
    ...
    std::stack<int> path;
    int io_num = insert_algorithm_->insert(*this, key, val, path);
    ...
    if (leaf->item_count == max_leaf_item) {
      ...
      while (!path.empty()) {
        ...
        Block* parent_block = cache_->get_block(parent_id);
        ...
        if (parent_node->item_count == max_inner_item - 1) {
          ...
          cache_->allocate_block(new_split_node_id, new_blk);
          cache_->write_block(parent_id);
  ...
  // restart:
    ...
    while (is_inner) {
      if (node->item_count == max_inner_item - 1) {
        ...
        KeyType sep;
        Block* new_blk = new Block();
        Block* old = cache_->get_block(cur_node_id);
        ...
        if (parent_node_id == -1) {
          ...
          Block* parent_block = cache_->get_block(parent_node_id);
          insert_inner(parent_block, sep, split_node_id);
          // sm_->write_block(parent_node_id, parent_block, &io_num);
          // buf.mark_dirty(parent_node_id);
          cache_->write_block(parent_node_id);
        }
        // goto restart;
        if (key <= sep) {
          // Do nothing
