...
struct BTree {
  ...
  struct InsertCost {
    std::vector<NodeBase*> read_set;
    std::vector<NodeBase*> write_set;
    double read_cost_;
    double write_cost_;

    void add_read(NodeBase* node) {
      if (std::find(read_set.begin(), read_set.end(), node) == read_set.end()) {
        burn_cpu(read_cost_);
        read_set.push_back(node);
      }
    }
    void add_dirty(NodeBase* node) {
    ...
    inline void burn_cpu(double microseconds) {
      ...
      while (std::chrono::duration<double, std::micro>(
                ...
                .count() < microseconds) {
          // small computation to prevent aggressive optimization
          asm volatile("" ::: "memory");
      }
    }
  };
  ...
  insert_cost.add_dirty(newInner);
  insert_cost.add_read(inner);
	if (parent) {
	  parent->insert(sep,newInner);
    insert_cost.add_dirty(parent);
	} else {
	  makeRoot(sep,inner,newInner);
	  insert_cost.add_dirty(root);
	}
	// Unlock and restart
	node->writeUnlock();
	if (parent)
  ...
  uint64_t bounded_scan(Key low_key, Key high_key, std::vector<Value>& output) {
    int restartCount = 0;
  restart:
    if (restartCount++)
      yield(restartCount);
    ...
    BTreeLeaf<Key,Value>* leaf = static_cast<BTreeLeaf<Key,Value>*>(node);
    BTreeLeaf<Key,Value>* prev_leaf = nullptr;
    ...
    while (!reached_end) {
      ...
      for (unsigned i = pos; i < leaf->count; i++) {
        ...
        output.push_back(leaf->payloads[i]);
      }
