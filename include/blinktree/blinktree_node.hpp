...
class inode_t: public node_t{
 ...
 public:
  inode_t() { } 
	...
	inode_t(node_t* sibling, int _cnt, node_t* left, uint32_t _level, Key_t _high_key)
  : node_t(sibling, _cnt, _level), high_key(_high_key) {
    entry[0].value = left;
	}

	// constructor when tree height grows
	inode_t(Key_t split_key, node_t* left, node_t* right,
          node_t* sibling, uint32_t _level, Key_t _high_key)
  : node_t(sibling, 1, _level) {
	    high_key = _high_key;
