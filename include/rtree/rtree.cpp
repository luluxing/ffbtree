...
Rectangle Merge(const Rectangle& rectangle1, const Rectangle& rectangle2) {
	return Rectangle(min(rectangle1.Left(), rectangle2.Left()),
...
pair<double, bool> SplitPerimSum(const vector<T*>& entries) {
	Rectangle prefix = MergeRange<T>(entries, 
                                   0, 
                                   TreeNode::minimum_entry - 1);
	Rectangle suffix = MergeRange<T>(entries, 
                                   TreeNode::maximum_entry - TreeNode::minimum_entry+1,
                                   entries.size());
	double perim_sum = 0;
	Rectangle rec_remaining;
	bool is_overlap = true;
	for(int idx = TreeNode::minimum_entry - 1;
      idx < TreeNode::maximum_entry - TreeNode::minimum_entry + 1;
      idx++) {
...
template<class T>
int FindMinimumSplit(
    const vector<T*>& entries,
    double(*score_func1) (const Rectangle &, const Rectangle &),
	  double(*score_func2)(const Rectangle &, const Rectangle &),
    double& min_value1, double& min_value2, Rectangle& rec1, Rectangle& rec2) {
...
template<class T>
int FindMinimumSplitRR(
    const vector<T*>& entries, 
    double ys, double y1, 
    double miu, double delta, 
    double perim_max, Rectangle& rec1, Rectangle& rec2) {
	Rectangle rec_prefix = MergeRange<T>(entries,
                                       0, 
                                       TreeNode::minimum_entry - 1);
	Rectangle rec_suffix = MergeRange<T>(entries,
                                       TreeNode::maximum_entry - TreeNode::minimum_entry +1,
                                       entries.size());
	int optimal_split = -1;
	Rectangle rec_remaining;
